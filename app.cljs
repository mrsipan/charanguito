;; Charanguito — Charango Chords & Tuner
...
;; ── Translations (i18n) ───────────────────────────────────────────
(def translations
  {:en {:app-title "Charanguito"
        :app-subtitle "Chords & Tuner"
        :tab-chords "Chords"
        :tab-progressions "Progressions"
        :tab-favorites "Favorites"
        :search-placeholder "Search chords..."
        :filter-all-roots "All Roots"
        :filter-all-types "All Types"
        :filter-all-levels "All Levels"
        :chords-found "chord(s) found"
        :no-chords "No charango chords match your search"
        :no-favorites "No favorites yet"
        :no-favorites-hint "Click the star on any chord to add it here."
        :play "Play"
        :play-progression "Play"
        :strum-chord "Strum Chord"
        :close "Close"
        :root "Root"
        :type "Type"
        :intervals "Intervals"
        ...
        :note "Note"
        :footer "chords"
        :footer-progressions "progressions"
        :footer-built "Built with Squint + Reagami + Bulma"
        :tab-tuner "Tuner"
        :tuner-start "Start Tuner"
        :tuner-stop "Stop Tuner"
        :tuner-play-ref "Play Ref"
        :tuner-need-mic "Microphone access is needed for the tuner."
        :tuner-mic-denied "Microphone access denied. Please allow mic access to use the tuner."
        :tuner-listening "Listening..."
        :tuner-course "Course"
        :tuner-target "Target"
        :tuner-cents "Cents"}
   :es {:app-title "Charanguito"
        :app-subtitle "Acordes y Afinador"
        :tab-chords "Acordes"
        :tab-progressions "Progresiones"
        :tab-favorites "Favoritos"
        :search-placeholder "Buscar acordes..."
        :filter-all-roots "Todas las Raíces"
        :filter-all-types "Todos los Tipos"
        :filter-all-levels "Todos los Niveles"
        :chords-found "acorde(s) encontrado(s)"
        :no-chords "No hay acordes de charango con esos filtros"
        :no-favorites "Sin favoritos aún"
        :no-favorites-hint "Haz clic en la estrella para agregar acordes."
        :play "Tocar"
        :play-progression "Tocar"
        :strum-chord "Rasguear Acorde"
        :close "Cerrar"
        :root "Raíz"
        :type "Tipo"
        ...
        :note "Nota"
        :footer "acordes de charango"
        :footer-progressions "progresiones"
        :footer-built "Hecho con Squint + Reagami + Bulma"
        :tab-tuner "Afinador"
        :tuner-start "Iniciar Afinador"
        :tuner-stop "Detener Afinador"
        :tuner-play-ref "Tocar Ref"
        :tuner-need-mic "Se necesita acceso al micrófono para el afinador."
        :tuner-mic-denied "Acceso al micrófono denegado. Permita el acceso para usar el afinador."
        :tuner-listening "Escuchando..."
        :tuner-course "Orden"
        :tuner-target "Objetivo"
        :tuner-cents "Cents"
        :note-names ["Do" "Do#" "Re" "Re#" "Mi" "Fa" "Fa#" "Sol" "Sol#" "La" "La#" "Si"]}
   :en-note-names ["C" "C#" "D" "D#" "E" "F" "F#" "G" "G#" "A" "A#" "B"]
   :es-note-names ["Do" "Do#" "Re" "Re#" "Mi" "Fa" "Fa#" "Sol" "Sol#" "La" "La#" "Si"]})

(defn tr [key] (get (get translations (get @app-state :lang :en)) key))

(defn note-display [note]
  "Translate a note name for display (C→Do in Spanish mode)."
  (let [idx (note-index note)
        names (get translations (if (= (get @app-state :lang :en) :es)
                                  :es-note-names
                                  :en-note-names))]
    (if (>= idx 0)
      (get names idx)
...
(defn transposed-note-display [note]
  "Display a note name translated, handling flat enharmonics."
  (note-display note))
...
;; ── Charango Tuner ─────────────────────────────────────────────────
(def tuner-stream (atom nil))
(def tuner-audio-ctx (atom nil))
(def tuner-anim-frame (atom nil))
(def tuner-detected (atom nil))
(def tuner-error (atom nil))

(def tuner-course-display ["1st (E-E)" "2nd (A-A)" "3rd (E-e)" "4th (C-C)" "5th (G-G)"])
(def tuner-target-notes ["E4" "A4" "E4" "C4" "G3"])

(defn cents-deviation [detected target]
  (if (and (> detected 0) (> target 0))
    (* 1200 (/ (Math/log (/ detected target)) (Math/log 2)))
    0))

(defn autocorrelation-pitch [buffer sample-rate]
  (let [min-lag (Math/floor (/ sample-rate 2500))
        max-lag (Math/floor (/ sample-rate 100))
        buf-len (.-length buffer)
        best-lag (atom min-lag)
        best-corr (atom -1e10)]
    (when (> buf-len max-lag)
      (dotimes [lag (- max-lag min-lag)]
        (let [actual-lag (+ min-lag lag)
              corr (atom 0.0)]
          (dotimes [i (- buf-len actual-lag)]
            (swap! corr + (* (aget buffer i) (aget buffer (+ i actual-lag)))))
          (when (> @corr @best-corr)
            (reset! best-corr @corr)
            (reset! best-lag actual-lag)))))
    (if (> @best-corr 0)
      (/ sample-rate @best-lag)
      0)))

(defn closest-course [freq]
  (let [best-idx (atom 0)
        best-dist (atom 1e10)]
    (dotimes [i 5]
      (let [target (aget course-base-freqs i)
            dist (Math/abs (- freq target))]
        (when (< dist @best-dist)
          (reset! best-dist dist)
          (reset! best-idx i))))
    @best-idx))

(defn tuner-play-ref [course-idx]
  (if-let [ctx (get-audio-ctx)]
    (let [freq (aget course-base-freqs course-idx)
          now (.-currentTime ctx)]
      (play-string ctx freq now 1.5 @volume))
    (println "Audio context not available")))

(defn stop-tuner! []
  (when-let [frame @tuner-anim-frame]
    (.cancelAnimationFrame js/window frame)
    (reset! tuner-anim-frame nil))
  (when-let [stream @tuner-stream]
    (.forEach (.getTracks stream) (fn [track] (.stop track)))
    (reset! tuner-stream nil))
  (when-let [ctx @tuner-audio-ctx]
    (.close ctx)
    (reset! tuner-audio-ctx nil))
  (reset! tuner-detected nil))

(defn tuner-loop [analyser sample-rate]
  (let [buffer-length (.-frequencyBinCount analyser)
        buffer (js/Float32Array. buffer-length)]
    (.getFloatTimeDomainData analyser buffer)
    (let [freq (autocorrelation-pitch buffer sample-rate)]
      (when (> freq 0)
        (let [cidx (closest-course freq)
              target (aget course-base-freqs cidx)
              cents (cents-deviation freq target)
              in-tune? (and (>= cents -10) (<= cents 10))]
          (reset! tuner-detected {:freq freq :cents cents :course-idx cidx :in-tune? in-tune?}))))
    (reset! tuner-anim-frame
      (.requestAnimationFrame js/window
        (fn [] (tuner-loop analyser sample-rate))))))
  ...
  (try
    (let [constraints #js {:audio true}]
      (-> (.getUserMedia (.-mediaDevices js/navigator) constraints)
          (.then (fn [stream]
                   (reset! tuner-stream stream)
                   (let [ctx (js/AudioContext.)
                         source (.createMediaStreamSource ctx stream)
                         analyser (.createAnalyser ctx)]
                     (set! (.-fftSize analyser) 4096)
                     (.connect source analyser)
                     (reset! tuner-audio-ctx ctx)
                     (reset! tuner-detected nil)
                     (tuner-loop analyser (.-sampleRate ctx)))))
          (.catch (fn [err]
                    (reset! tuner-error (tr :tuner-mic-denied))
                    (println "Mic error:" (.-message err))))))
    (catch js/Error e
      (reset! tuner-error (tr :tuner-need-mic))
      (println "Tuner start error:" (.-message e)))))

(defn tuner-row [course-idx detected]
  (let [target-freq (aget course-base-freqs course-idx)
        target-note (aget tuner-target-notes course-idx)
        course-name (aget tuner-course-display course-idx)
        is-active (and detected (= (get detected :course-idx) course-idx))
        cents (if is-active (get detected :cents) 0)
        in-tune? (if is-active (get detected :in-tune?) false)
        bar-pos (max 0 (min 100 (+ 50 (* cents 1.0))))
        bar-color (if in-tune? "#27ae60"
                     (if (and (>= cents -15) (<= cents 15)) "#f39c12"
                       "#e74c3c"))]
    [:div {:class (str "box tuner-row mb-2 " (if in-tune? "has-background-success-light" ""))
           :key (str "tuner-" course-idx)}
     [:div {:class "columns is-mobile is-align-items-center is-gapless"}
      [:div {:class "column is-2"}
       [:p {:class "has-text-weight-bold"} course-name]
       [:p {:class "is-size-7 has-text-grey"} target-note (str " (" (.toFixed target-freq 1) " Hz)")]]
      [:div {:class "column is-1 has-text-centered"}
       (if is-active
         [:span {:class "tag is-medium" :style (str "background:" bar-color ";color:white;")}
          (if (> cents 0) "+" "") (.toFixed cents 1) "¢"]
         [:span {:class "tag is-light"} "—"])]
      [:div {:class "column is-7"}
       [:div {:style "background:#eee;border-radius:4px;height:24px;position:relative;overflow:hidden;"}
        [:div {:style (str "background:" bar-color ";height:100%;width:" bar-pos "%;transition:width 0.1s;")}]
        [:div {:style "position:absolute;left:50%;top:0;width:2px;height:100%;background:#333;"}]]]
      [:div {:class "column is-2 has-text-right"}
       [:button {:class "button is-small is-info"
                 :onclick (fn [_] (tuner-play-ref course-idx))}
        (str "▶ " (tr :tuner-play-ref))]]]]))

(defn tuner-view [state]
  (let [detected @tuner-detected
        running? (some? @tuner-stream)
        error-msg @tuner-error]
    [:div
     [:div {:class "level mb-4"}
      [:div {:class "level-left"}
       [:h2 {:class "title is-4"} "🎵 " (tr :tab-tuner)]]
      [:div {:class "level-right"}
       (if running?
         [:button {:class "button is-danger"
                   :onclick (fn [_] (stop-tuner!))}
          (tr :tuner-stop)]
         [:button {:class "button is-success"
                   :onclick (fn [_]
                              (reset! tuner-error nil)
                              (start-tuner!))}
          (tr :tuner-start)])]]
     (when error-msg
       [:div {:class "notification is-warning"}
        [:button {:class "delete" :onclick (fn [_] (reset! tuner-error nil))}]
        error-msg])
     (when running?
       [:div {:class "has-text-centered mb-3"}
        [:span {:class "tag is-info is-medium"} "🎤 " (tr :tuner-listening)]])
     (into [:div]
           (mapv (fn [i] (tuner-row i detected))
                 (range 5)))]))
         ...
         [:div {:class "modal-card" :style "max-width:500px;"
          ...
          [:section {:class "modal-card-body"}
           ...
           [:div {:class "content mt-3"}
            [:p [:strong (str (tr :root) ": ")] (get chord-data :root)]
            [:p [:strong (str (tr :type) ": ")] (get chord-data :type)]
            [:p [:strong (str (tr :intervals) ": ")] (str (get chord-data :intervals))]
            [:p [:strong (str (tr :difficulty) ": ")] (difficulty-tag (get chord-data :difficulty))]
            (into [:table {:class "table is-striped is-narrow is-fullwidth"}]
                  (into [:thead [:tr [:th (tr :course)] [:th "Note"] [:th (tr :fret)] [:th (tr :finger)]]]
                        [(into [:tbody]
                               (mapv (fn [i]
                                       (let [s (get fingering i)
                                         ...
                                         [:tr {:key (str "fret-" i)}
                                          ...
                                          [:td (if (= fret -1) [:span {:style "color:#e74c3c"} (tr :muted)]
                                                   (if (= fret 0) [:span {:style "color:#27ae60"} (tr :open-str)] fret))]
    ...
    [:div
     [:div {:class "field is-horizontal is-flex-wrap-wrap"}
      [:div {:class "field-body mr-2"}
       [:p {:class "control has-icons-left"}
        [:input {:class "input" :type "text" :placeholder (tr :search-placeholder)
      ...
      [:div {:class "field mr-2"}
       [:div {:class "select"}
        (into [:select {:value (get state :filter-root)
               ...
               [:option {:value "All"} (tr :filter-all-roots)]]
              (mapv (fn [n] [:option {:value n :key n} (note-display n)]) all-notes))]]
      [:div {:class "field mr-2"}
       [:div {:class "select"}
        (into [:select {:value (get state :filter-type)
                        :onchange (fn [e] (swap! app-state assoc :filter-type (.. e -target -value)))}
               [:option {:value "All"} (tr :filter-all-types)]]
     ...
     (when (zero? (count filtered))
       [:div {:class "has-text-centered has-text-grey py-6"}
        [:p {:class "is-size-4"} [:img {:src "charanguito.jpg" :style "height:48px;border-radius:6px;"}]]
        [:p (tr :no-chords)]])]))
    ...
    [:div
     ...
     [:div {:class "field is-horizontal is-align-items-center mb-4"}
      [:div {:class "field-label is-normal"}
       [:label {:class "label"} (tr :transpose)]]
...
(defn favorites-view [state]
  (let [favs (get state :favorites)]
    (if (empty? favs)
      [:div {:class "has-text-centered has-text-grey py-6"}
       [:p {:class "is-size-3"} "⭐"]
       [:p {:class "is-size-5"} (tr :no-favorites)]
       [:p (tr :no-favorites-hint)]]
      [:div
       [:p {:class "has-text-grey is-size-7 mb-3"}
        (str (count favs) " " (tr :favorites-count))]
       (into [:div {:class "columns is-multiline is-mobile"}]
             (mapv (fn [k]
    ...
    [:nav {:class (str "navbar " (if dark "is-dark" "is-light")) :role "navigation"}
     [:div {:class "navbar-brand"}
      [:a {:class "navbar-item"
       ...
       [:span {:class "is-size-7 has-text-grey ml-2"} (tr :app-subtitle)]]
     ...
     [:div {:class "navbar-menu"}
      [:div {:class "navbar-start"}
       ...
       [:a {:class (str "navbar-item " (if (= active "favorites") "is-active" ""))
        ...
        (str (tr :favorites-count) " (" (count (get state :favorites)) ")")]
       [:a {:class (str "navbar-item " (if (= active "tuner") "is-active" ""))
            :onclick (fn [_] (swap! app-state assoc :active-tab "tuner"))}
        (tr :tab-tuner)]]
      [:div {:class "navbar-end"}
       ;; Settings dropdown
       [:div {:class "navbar-item has-dropdown is-hoverable"}
        [:a {:class "navbar-link"} "⚙️"]
        [:div {:class "navbar-dropdown is-right"}
         ...
         [:div {:class "navbar-item"}
          [:button {:class (str "button is-small " (if (get state :octave-mode) "is-success" "is-light"))
           ...
           (str "🎵 " (if (get state :octave-mode) (tr :octave) (tr :unison)))]]
         ...
         [:div {:class "navbar-item"}
          [:div {:style "display:flex;align-items:center;gap:6px;"}
           ...
           [:input {:class "slider" :type "range" :min "0" :max "1" :step "0.05"
                    :value (get state :volume) :style "width:70px;"
                    :oninput (fn [e]
                               (let [v (js/parseFloat (.. e -target -value))]
                                 (swap! app-state assoc :volume v)
                                 (reset! volume v)))}]
           [:span {:class "tag is-info is-size-7"} (int (* (get state :volume) 100)) "%"]]]
         [:hr {:class "navbar-divider"}]
         ;; Language
         [:div {:class "navbar-item"}
          [:button {:class "button is-small is-light"
                    :onclick (fn [_] (swap! app-state update :lang #(if (= % :en) :es :en)))}
           (if (= (get state :lang) :en) "🇪🇸 ES" "🇬🇧 EN")]]
         ;; Theme
         [:div {:class "navbar-item"}
          [:button {:class (str "button is-small " (if dark "is-warning" "is-dark"))
                    :onclick (fn [_] (swap! app-state update :dark-theme not))}
           (if dark (str "☀ " (tr :light)) (str "🌙 " (tr :dark)))]]]]]]]))
...
(defn app-ui [state]
  (let [active (get state :active-tab)
    ...
    [:div {:class (if dark "has-background-dark has-text-light" "")}
     ;; Logo banner
     [:div {:class "has-text-centered py-3" :style "background: linear-gradient(135deg, #f5e6d3 0%, #e8d5b7 50%, #f0e6d8 100%);"}
